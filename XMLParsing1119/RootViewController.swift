...
class RootViewController: UITableViewController, XMLParserDelegate {
    ...
    func parser(_ parser: XMLParser, didStartElement elementName: String, namespaceURI: String?, qualifiedName qName: String?, attributes attributeDict: [String : String] = [:]){
        ...
        if elementName == "Book"{
            book = Book()
            //Book 태그에 있느 id 속성의 값을 찾아서 book에 저장
    ...
    //이 메소드는 유일하게 동일한 데이터를 가지고 두 번 이상 호추로딜 수 있는 메소드.
    //elementValue의 값이 nil이면 바로 저장하고 그렇지 않으면 이전 데이터에 추가해야 함.
    //태그에 적는 내용은 하나의 패킷 이하로 만들어지기 때문에 한번에 전송되지만
    //여는 태그와 닫는 태그 사이에 작성하는 내용은 하나의 패킷 이상이 될 수 있기 때문이다.
    //이 부분은 다른 네트워크 프로그램을 만들 떄도 기억해야 한다.
